...
const { log } = prepare("RouterClassStack");
...
// component name
const component: string = "RouterClassStack";

/**
 * @name RouterClassStack
 */
export default class RouterClassStack extends Component<IProps, IStates> {
  /**
  ...
  protected async sequential(pCurrentRoute: IRoute) {
    ...
    await pagesTransitionsList?.[
      this.state.oldRoute?.componentName
    ]?.playOut?.();
    ...
    await pagesTransitionsList?.[
      this.state.currentRoute?.componentName
    ]?.playIn?.();
    // toggle playing state
